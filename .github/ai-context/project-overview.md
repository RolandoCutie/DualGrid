...
| Feature                        | Description                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Landing Page**               | Hero with dual CTAs (web + branding), service overview, portfolio, process, unified plan cards for web/branding/hosting/domain, testimonials, and final CTA. The landing copy uses a more conversion-focused, professional and human voice centered on clarity, dedication, professionalism and carefully executed work. Dark mode uses a cinematic multi-layer neon gradient background across all landing sections.       |
...
| **Branding Questionnaire**     | 4-question scoring quiz (A/B/C per question). Progress now renders as dynamic `Paso/Step X de/of Y` instead of hardcoded “Bloque” labels in the modal flow. Recommends one of 3 branding plans: Grid Essential ($100), Grid Corporate ($300), Grid Ecosystem ($550). Admin creates shareable token links and sends them to clients via WhatsApp. Results stored in `BrandingQuestionnaire` collection.                      |
| **Bilingual UI (i18n)**        | EN/ES via `LanguageProvider`. Strings in `app/i18n/[locale].json`.                                                                                                                                                                                                                                                                                                                                                          |
| **Dark / Light Mode**          | **Dark mode only** — ThemeProvider always returns 'dark'. No toggle exists. Palette: #00D9FF (primary/cyan), #00FF9D (accent/green), #2B303A (card), #253C78 (secondary/navy), #A594F9 (purple).                                                                                                                                                                                                                            |
